...
PS7Area TS7Worker::GetArea(byte S7Code, word index)
{
    switch(S7Code)
    {
    case S7AreaPE : return FServer->FDevice->PE;
    case S7AreaPA : return FServer->FDevice->PA;
    case S7AreaMK : return FServer->FDevice->MK;
    case S7AreaCT : return FServer->FDevice->CT;
    case S7AreaTM : return FServer->FDevice->TM;
    case S7AreaDB : return FServer->FDevice->DB->Find(index);
    default       : return NULL;
    };
}
//------------------------------------------------------------------------------
...
bool TS7Worker::PerformFunctionUpload()
{
    ...
    area = FServer->FDevice->getArea(BlkType)->Find(BlkNum);
...
bool TS7Worker::PerformFunctionDownload()
{
    ...
    if (last_packet && written == BlkLen) {
        FServer->FDevice->getArea(BlkType)->Remove(BlkNum);
        PS7Area NewArea = new TS7Area(BlkNum, BlkLen);
        if (FServer->FDevice->getArea(BlkType)->Add(NewArea)) {
            RetCode = evrMemoryLimit;
            delete NewArea;
        } else {
            RetCode = evrNoError;
        }
        ...
        *((word*)&ResData->Data[8]) = SwapWord(item_count);
        ...
        for (word i = 0; i < item_count; i++) {
            ...
            switch (item.getMemoryArea()) {
                case VT_M:
                    memory_area = FServer->FDevice->MK;
                    break;
                case VT_E:
                    memory_area = FServer->FDevice->PE;
                    break;
                case VT_A:
                    memory_area = FServer->FDevice->PA;
                    break;
                case VT_DB:
                    memory_area = FServer->FDevice->DB->Find(item.db_number);
                    break;
...
void TS7Worker::BLK_ListAll(TCB &CB)
{
    ...
    Data->Blocks[0].BCount=SwapWord(FServer->FDevice->OB->AreaCount);
    Data->Blocks[1].Zero=0x30;
    Data->Blocks[1].BType=Block_FB;
    Data->Blocks[1].BCount=SwapWord(FServer->FDevice->FB->AreaCount);
    Data->Blocks[2].Zero=0x30;
    Data->Blocks[2].BType=Block_FC;
    Data->Blocks[2].BCount=SwapWord(FServer->FDevice->FC->AreaCount);
    Data->Blocks[3].Zero=0x30;
    Data->Blocks[3].BType=Block_DB;
    Data->Blocks[3].BCount=SwapWord(FServer->FDevice->DB->AreaCount);
    Data->Blocks[4].Zero=0x30;
    Data->Blocks[4].BType=Block_SDB;
    Data->Blocks[4].BCount=SwapWord(FServer->FDevice->SDB->AreaCount);
...
void TS7Worker::BLK_ListBoT(byte BlockType, bool Start, TCB &CB)
{
    ...
    if ((AreaCount = FServer->FDevice->getArea(BlockType)->AreaCount) > 0) {
        MaxIndex = FServer->FDevice->getArea(BlockType)->MaxIndex;
        PS7AreaContainer AreaContainer = FServer->FDevice->getArea(BlockType);
        Data->RetVal=0xFF;
        Data->TSize =TS_ResOctet;
        CB.DataLength= 4 + (AreaCount * word(sizeof(TDataFunGetBotItem)));
        CB.Answer.Header.DataLen=SwapWord(CB.DataLength);
        Data->DataLen=SwapWord(CB.DataLength-4);
        for (int i = 0; i <= MaxIndex; i++) {
            PS7Area Area = AreaContainer->Get(i);
            if (Area == NULL)
                continue;
            Data->Items[i].BlockNum = SwapWord(Area->Number);
            Data->Items[i].Unknown = uk;
            Data->Items[i].BlockLang = blockLang;
        };
    }
    else
...
void TS7Worker::BLK_GetBlkInfo(TCB &CB)
{
    ...
    PS7AreaContainer AreaContainer = FServer->FDevice->getArea(ReqData->BlkType);
    if (AreaContainer != NULL) {
        PS7Area Area = AreaContainer->Find(AsciiToNum(ReqData->AsciiBlk, sizeof(ReqData->AsciiBlk)));
        if (Area != NULL) {
            memcpy(&Data->Cst_pp, Area->PData, sizeof(TS7CompactBlockInfo));
            memcpy(&Data->Author, Area->PData + Area->Size - 36, 36);
...
{
    longword items = FServer->GetDiagItemCount();
    ...
    ResData->DLen = SwapWord(longword(bufferSize + 8));
    ResData->ID = 0xA000;
...
// S7 SERVER CLASS
//------------------------------------------------------------------------------
TSnap7Server::TSnap7Server(TPlcMemory *Device)
{
    FDevice = Device;
    OnReadEvent=NULL;
...
//------------------------------------------------------------------------------
longword TSnap7Server::GetDiagItemCount() {
...
//------------------------------------------------------------------------------
int TSnap7Server::SetForcePDU(word PduSize)
{
...
pbyte TSnap7Server::GetBlock(byte BlkType, word BlkNum) {
    TS7AreaContainer *areaContainer = FDevice->getArea(BlkType);
    if (!areaContainer)
        return NULL;
