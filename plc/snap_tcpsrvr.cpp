...
void TMsgWorkerThread::Execute()
{
    ...
    if (!FServer->Destroying)
    {
        ...
        if (Exception)
        {
            ...
            FServer->DoEvent(WorkerSocket->ClientHandle, evcClientException, 0, FServer->LocalPort, 0, 0, 0);
        }
        else
            if (SelfClose)
        {
            FServer->DoEvent(WorkerSocket->ClientHandle, evcClientDisconnected, 0, FServer->LocalPort, 0, 0, 0);
        }
        else
            FServer->DoEvent(WorkerSocket->ClientHandle, evcClientTerminated, 0, FServer->LocalPort, 0, 0, 0);
    }
    delete WorkerSocket;
...
void TCustomMsgServer::Incoming(socket_t Sock)
{
    ...
    if (CanAccept(Sock))
    {
        ...
        if (idx >= 0)
        {
            ...
            DoEvent(WorkerSocket->ClientHandle, evcClientAdded, 0, LocalPort, 0, 0, 0);
        }
        else
        {
            DoEvent(ClientHandle, evcClientNoRoom, 0, LocalPort, 0, 0, 0);
            Msg_CloseSocket(Sock);
        }
        UnlockList();
    }
    ...
    {
        Msg_CloseSocket(Sock);
        DoEvent(ClientHandle, evcClientRejected, 0, LocalPort, 0, 0, 0);
    };
}
//---------------------------------------------------------------------------
int TCustomMsgServer::Start()
{
    ...
    if (Status != SrvRunning)
    {
        ...
        if (Result != 0)
        {
            DoEvent(0, evcListenerCannotStart, Result, 0, LocalPort, 0, 0);
            Status = SrvError;
        }
        else
        {
            DoEvent(0, evcServerStarted, 0, LocalPort, 0, 0, 0);
            Status = SrvRunning;
        };
    };
    FLastError = Result;
